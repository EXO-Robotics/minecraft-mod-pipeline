...
ck(bp["header"]["version"]==[1,1,1] and rp["header"]["version"]==[1,1,1],"pack version");ck(rp["header"].get("pack_scope")=="world","RP world scope")
ck({d.get("module_name"):d.get("version") for d in bp["dependencies"]}.get("@minecraft/server")=="2.0.0","stable API")
ck(main.count("[Aionbound Core Beta] runtime-ready-g6")==1 and main.index("runtime-ready-g6")<main.index("startRuntime()"),"literal entry initialization")
...
for token in ["function cellEdits","minecraft:stone","minecraft:air","aionbound:orevein_hollow_gate","cell.return.location","cursor:interrupted.cursor","encounters:{active:{}","state:\"admitted\"","delete j.active[key]","glasswing:first_defeat","Defeat Glasswing before foundry admission"]:ck(token in rt,"repair invariant "+token)
for name in ["glasswing_sentinel","royal_moth_empress","chrono_robo_sentinel","basalt_behemoth","rift_colossus","ash_sovereign_wyrm","tide_empress_wyrm"]:
 c=json.loads((R/f"behavior_pack/entities/{name}.entity.json").read_text())["minecraft:entity"]["components"];ck("minecraft:behavior.nearest_attackable_target" in c and "minecraft:behavior.hurt_by_target" in c and "minecraft:behavior.melee_attack" in c,"combat targeting "+name)
mount=json.loads((R/"behavior_pack/entities/waykeeper_courser.entity.json").read_text())["minecraft:entity"]["components"];ck("minecraft:rideable" in mount and "minecraft:input_ground_controlled" in mount and "minecraft:behavior.controlled_by_player" not in mount and bool(mount["minecraft:rideable"].get("interact_text")),"direct-input mount")
...
for pkg in ["dist/aionbound-core-beta-g6-behavior.mcpack","dist/aionbound-core-beta-g6-resources.mcpack"]:
...
with zipfile.ZipFile(R/"dist/aionbound-core-beta-g6.mcaddon") as z:ck(z.namelist()==["aionbound-core-beta-g6-behavior.mcpack","aionbound-core-beta-g6-resources.mcpack"],"exact addon membership")
...
print("Aionbound Core Beta generation 6 producer-local validation: PASS")
