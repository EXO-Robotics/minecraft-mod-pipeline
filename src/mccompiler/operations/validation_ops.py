...
def start_test_runtime(store: ProjectStore, parameters: dict[str, Any], expected_revision: int | None = None) -> HandlerResult:
    ...
    probes: list[BDSConsoleProbe] = []
    required_probe_fields = {"check_id", "cycle", "after_boot_seconds", "command", "expect_output"}
    for index, raw in enumerate(raw_probes):
        if not isinstance(raw, dict) or set(raw) != required_probe_fields:
            raise OperationError("INVALID_PARAMETERS", f"console_probes[{index}] must contain exactly {sorted(required_probe_fields)}")
        if not isinstance(raw["cycle"], int) or isinstance(raw["cycle"], bool):
            raise OperationError("INVALID_PARAMETERS", f"console_probes[{index}].cycle must be an integer")
        delay = raw["after_boot_seconds"]
        if not isinstance(delay, (int, float)) or isinstance(delay, bool):
            raise OperationError("INVALID_PARAMETERS", f"console_probes[{index}].after_boot_seconds must be numeric")
        if not all(isinstance(raw[field], str) for field in ("check_id", "command", "expect_output")):
            raise OperationError("INVALID_PARAMETERS", f"console_probes[{index}] string fields must be strings")
        probes.append(BDSConsoleProbe(
