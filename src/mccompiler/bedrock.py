...
EVENT_SYMBOLS = {
    ...
    "item_use_on_block": ("playerInteractWithBlock", "world.beforeEvents.playerInteractWithBlock"),
    "block_interact": ("playerInteractWithBlock", "world.beforeEvents.playerInteractWithBlock"),
...
def _script_modules(ir: dict[str, Any], plan: dict[str, Any], *, debug: bool) -> tuple[dict[str, Any], list[dict[str, Any]], dict[str, str], list[dict[str, str]]]:
    ...
    for behavior in approved:
        ...
        if any(action.get("type") == "spawn_projectile" for action in behavior.get("actions", [])):
            requirements.update({
                ("@minecraft/server", "Entity.getHeadLocation"),
                ("@minecraft/server", "Entity.getViewDirection"),
                ("@minecraft/server", "EntityProjectileComponent.owner"),
...
const stateOwner=c=>c.block?world:(actor(c)||target(c)||world);
...
export function dispatch(behavior,c={}){if(!conditionsPass(behavior,c))return false;const a=actor(c),t=target(c),d=dimension(c,a,t),p=location(c,a,t);for(const x of behavior.actions||[]){if(x.condition&&!conditionPass(x.condition,behavior,c))continue;switch(x.type){case 'spawn_entity':d.spawnEntity(x.entity||'minecraft:pig',p);break;case 'spawn_projectile':{const view=vector(a?.getViewDirection?.(),{x:0,y:0,z:1}),velocity=vector(x.velocity,scale(view,x.speed||1));const head=a?.getHeadLocation?.()||p,origin={x:head.x+view.x*0.6,y:head.y+view.y*0.6,z:head.z+view.z*0.6};const e=d.spawnEntity(x.entity||'minecraft:snowball',origin),projectile=e.getComponent?.('minecraft:projectile');if(!projectile?.shoot)throw new Error(`[mccompiler] generated projectile ${x.entity||'minecraft:snowball'} lacks minecraft:projectile`);if(a)projectile.owner=a;projectile.shoot(velocity);break}case 'remove_entity':recipient(x,a,t)?.remove?.();break;case 'create_explosion':d.createExplosion(p,x.power||1,{breaksBlocks:x.breaks_blocks??false,source:a});break;case 'damage':recipient(x,a,t)?.applyDamage?.(x.amount||1);break;case 'heal':{const h=recipient(x,a,t)?.getComponent?.('minecraft:health');h?.setCurrentValue?.(Math.min(h.effectiveMax,h.currentValue+(x.amount||1)));break}case 'apply_effect':recipient(x,a,t)?.addEffect?.(effectId(x.effect),x.duration||20,{amplifier:x.amplifier||0});break;case 'remove_effect':recipient(x,a,t)?.removeEffect?.(effectId(x.effect));break;case 'play_sound':d.playSound(x.sound||'random.orb',p);break;case 'spawn_particles':d.spawnParticle(x.particle||'minecraft:basic_flame_particle',p,new MolangVariableMap());break;case 'set_block':case 'replace_block':c.block?.setType?.(x.block||'minecraft:stone');break;case 'break_block':c.block?.setType?.('minecraft:air');break;case 'place_structure':d.runCommand?.(`structure load ${x.structure||'mccompiler:placeholder'} ${p.x} ${p.y} ${p.z}`);break;case 'teleport':a?.teleport?.(x.location||p,{dimension:d});break;case 'apply_velocity':a?.applyImpulse?.(x.velocity||{x:0,y:0,z:0});break;case 'modify_item_durability':{const durability=c.itemStack?.getComponent?.('minecraft:durability');if(durability)durability.damage=Math.max(0,durability.damage+(x.amount||1));break}case 'add_item':{const item=new ItemStack(x.item||'minecraft:stone',x.amount||1);const bag=a?.getComponent?.('minecraft:inventory')?.container;if(bag)bag.addItem(item);else d.spawnItem(item,{x:p.x,y:p.y+1,z:p.z});break}case 'remove_item':{const bag=a?.getComponent?.('minecraft:inventory')?.container;if(bag)for(let i=0;i<bag.size;i++){const item=bag.getItem(i);if(item?.typeId===(x.item||'minecraft:stone')){bag.setItem(i);break}}break}case 'update_persistent_state':{const current=Number(readState(behavior,c,x.key||behavior.id));const value=x.operation==='increment'?current+(x.amount||1):x.operation==='decrement'?current-(x.amount||1):(x.value??1);writeState(behavior,c,x.key||behavior.id,value);break}case 'start_cooldown':a?.startItemCooldown?.(x.category||behavior.id,x.ticks||20);break;case 'set_entity_phase':recipient(x,a,t)?.setDynamicProperty?.('mccompiler:phase',x.value||1);console.warn(`[mccompiler] phase ${behavior.id} -> ${x.value||1}`);break;case 'trigger_behavior':system.run(()=>dispatch({...behavior,id:x.behavior||behavior.id,actions:x.actions||[]},c));break;case 'send_player_feedback':a?.sendMessage?.(x.message||behavior.id);break;case 'open_interaction_ui':if(a)system.run(()=>openGeneratedForm(a,x.ui||behavior.id));break;case 'schedule_delayed_action':system.runTimeout(()=>dispatch({...behavior,actions:x.actions||[]},c),x.ticks||1);break;default:console.warn(`[mccompiler] unresolved runtime action ${x.type} in ${behavior.id}`);break;}}return true;}
""",
    ...
    modules["scripts/events/generated.js"] = modules["scripts/events/generated.js"].replace(
        ...
        "if(matches(b,ctx)){if(!contextComplete(b,ctx)){console.warn(`[mccompiler] skipped ${b.id} missing required event context: ${missingContext(b,ctx).join(',')}`);return;}system.run(()=>dispatch(b,ctx));}",
    ).replace(
        "const matches=(b,c)=>",
        "const matches=(b,c)=>",
    ).replace(
    ...
    modules["scripts/runtime/actions.js"] = modules["scripts/runtime/actions.js"].replace(
        "const scale=(value,magnitude)=>({x:value.x*magnitude,y:value.y*magnitude,z:value.z*magnitude});",
        "const scale=(value,magnitude)=>({x:value.x*magnitude,y:value.y*magnitude,z:value.z*magnitude});\nconst normalize=value=>{const length=Math.hypot(value.x,value.y,value.z)||1;return scale(value,1/length)};",
    ).replace(
        "velocity=vector(x.velocity,scale(view,x.speed||1));const head=",
        "velocity=vector(x.velocity,scale(view,x.speed||1)),direction=normalize(velocity);const head=",
    ).replace(
        "origin={x:head.x+view.x*0.6,y:head.y+view.y*0.6,z:head.z+view.z*0.6}",
        "origin={x:head.x+direction.x*0.6,y:head.y+direction.y*0.6,z:head.z+direction.z*0.6}",
    ).replace(
    ...
    modules["scripts/runtime/actions.js"] = modules["scripts/runtime/actions.js"].replace(
        "export function readState(behavior,c,key){return stateOwner(c).getDynamicProperty?.(stateKey(behavior,c,key))??0}",
        "export function readState(behavior,c,key){try{return stateOwner(c).getDynamicProperty?.(stateKey(behavior,c,key))??0}catch(error){console.warn(`[mccompiler] unavailable state owner read ${String(error)}`);return 0}}",
    ).replace(
        "export function writeState(behavior,c,key,value){stateOwner(c).setDynamicProperty?.(stateKey(behavior,c,key),value)}",
        "export function writeState(behavior,c,key,value){try{stateOwner(c).setDynamicProperty?.(stateKey(behavior,c,key),value)}catch(error){console.warn(`[mccompiler] unavailable state owner write ${String(error)}`)}}",
    )
