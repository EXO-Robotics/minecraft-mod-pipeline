...
""",
    ...
    modules["scripts/events/generated.js"] = modules["scripts/events/generated.js"].replace(
        ...
        "const matches=(b,c)=>",
        "const contextComplete=(type,c)=>type==='item_use'?!!c.source:type==='item_use_on_block'||type==='block_interact'||type==='block_break'?!!(c.player||c.source)&&!!c.block:type==='entity_hit'?!!c.damagingEntity&&!!c.hitEntity:type==='entity_hurt'?!!c.hurtEntity:type==='entity_death'?!!c.deadEntity:type==='projectile_impact'?!!c.projectile&&!!c.hitEntity:type==='player_join'?!!c.player:type==='entity_spawn'?!!c.entity:true;\nconst matches=(b,c)=>",
    ).replace(
