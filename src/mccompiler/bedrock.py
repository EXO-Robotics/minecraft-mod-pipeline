...
const location=(c,a,t)=>a?.location||t?.location||c.block?.location||{x:0,y:0,z:0};
const dimension=(c,a,t)=>a?.dimension||t?.dimension||c.block?.dimension||world.getDimension('overworld');
export function conditionsPass(behavior,c){const a=actor(c),t=target(c);return (behavior.conditions||[]).every(x=>{switch(x.type){case 'player_sneaking':return !!a?.isSneaking;case 'held_item_match':return c.itemStack?.typeId===x.identifier;case 'target_entity_match':return t?.typeId===x.identifier;case 'block_match':return c.block?.typeId===x.identifier;case 'dimension_match':return a?.dimension?.id===x.identifier;case 'random_probability':return Math.random()<(x.probability??1);case 'cooldown_ready':return (a?.getItemCooldown?.(x.category||behavior.id)||0)===0;case 'health_threshold':return true;case 'client_server_side':return x.side!=='client';case 'configuration_flag':return x.enabled!==false;case 'dependency_presence':return x.present!==false;default:return true;}})}
export function dispatch(behavior,c={}){if(!conditionsPass(behavior,c))return false;const a=actor(c),t=target(c),d=dimension(c,a,t),p=location(c,a,t);for(const x of behavior.actions||[]){switch(x.type){case 'spawn_entity':d.spawnEntity(x.entity||'minecraft:pig',p);break;case 'spawn_projectile':{const e=d.spawnEntity(x.entity||'minecraft:snowball',p);e.applyImpulse?.(x.velocity||{x:0,y:0,z:1});break}case 'remove_entity':t?.remove?.();break;case 'create_explosion':d.createExplosion(p,x.power||1,{breaksBlocks:x.breaks_blocks??false,source:a});break;case 'damage':t?.applyDamage?.(x.amount||1);break;case 'heal':{const h=t?.getComponent?.('minecraft:health');h?.setCurrentValue?.(Math.min(h.effectiveMax,h.currentValue+(x.amount||1)));break}case 'apply_effect':(t||a)?.addEffect?.(x.effect||'speed',x.duration||20,{amplifier:x.amplifier||0});break;case 'remove_effect':(t||a)?.removeEffect?.(x.effect||'speed');break;case 'play_sound':d.playSound(x.sound||'random.orb',p);break;case 'set_block':case 'replace_block':c.block?.setType?.(x.block||'minecraft:stone');break;case 'break_block':c.block?.setType?.('minecraft:air');break;case 'place_structure':d.runCommand?.(`structure load ${x.structure||'mccompiler:placeholder'} ${p.x} ${p.y} ${p.z}`);break;case 'teleport':a?.teleport?.(x.location||p,{dimension:d});break;case 'apply_velocity':a?.applyImpulse?.(x.velocity||{x:0,y:0,z:0});break;case 'add_item':a?.getComponent?.('minecraft:inventory')?.container?.addItem?.(new ItemStack(x.item||'minecraft:stone',x.amount||1));break;case 'update_persistent_state':(a||world).setDynamicProperty?.(x.key||behavior.id,x.value??1);break;case 'start_cooldown':a?.startItemCooldown?.(x.category||behavior.id,x.ticks||20);break;case 'set_entity_phase':(t||a)?.setDynamicProperty?.('mccompiler:phase',x.value||1);break;case 'trigger_behavior':system.run(()=>dispatch({...behavior,id:x.behavior||behavior.id,actions:x.actions||[]},c));break;case 'send_player_feedback':a?.sendMessage?.(x.message||behavior.id);break;case 'schedule_delayed_action':system.runTimeout(()=>dispatch({...behavior,actions:x.actions||[]},c),x.ticks||1);break;default:console.warn(`[mccompiler] unresolved runtime action ${x.type} in ${behavior.id}`);break;}}return true;}
""",
