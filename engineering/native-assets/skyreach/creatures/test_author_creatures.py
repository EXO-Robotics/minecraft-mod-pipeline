...
CONTACTS_SPEC = importlib.util.spec_from_file_location("skyreach_creature_contacts", HERE / "build_contact_sheets.py")
...
class SkyreachCreatureNativeTest(unittest.TestCase):
    def test_scope_and_clip_total_are_exact(self):
        self.assertEqual(list(author.SPECS), ["cliff_ram", "glide_drake", "ropewing", "ruin_harpy", "sky_fox", "stone_vulture", "storm_gull"])
        self.assertEqual(sum(len(spec["clips"]) for spec in author.SPECS.values()), 36)
    ...
    def test_native_receipts_and_proofs(self):
        for asset, spec in author.SPECS.items():
            ...
            self.assertEqual(receipt["schema"], "aionforge.wave1.skyreach.creature_native.v1")
    ...
    def test_aggregate_is_deterministic(self):
        ...
        self.assertEqual(report["totals"], {"assets": 7, "brief_declared_clips": 36, "true_native_locators": 14, "screenshots": 92, "warnings": 0, "errors": 0})

    def test_contact_sheets_bind_exact_native_screenshot_inventories(self):
        manifest = json.loads((HERE / "SKYREACH_CREATURE_NATIVE_CONTACT_SHEETS.json").read_text())
        self.assertEqual(manifest["schema"], "aionforge.wave1.skyreach.creature_native_contact_sheets.v1")
