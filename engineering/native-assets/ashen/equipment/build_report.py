#!/usr/bin/env python3
"""Build the deterministic aggregate receipt for Ashen-facing equipment."""
...
OUTPUT = HERE / "ASHEN_EQUIPMENT_NATIVE_REPORT.json"
...
def build() -> dict:
    ...
    return {
        "schema": "aionforge.wave1.ashen.equipment_native.aggregate.v1",
        ...
        "integration_authority": {"commit": author.INTEGRATION_COMMIT},
        "scope": list(ASSETS),
        "excluded_and_unchanged": {"briar_ring": "EXISTING_WHISPERWOOD_BASE_W1_CREATIVE_005_DEFERRED"},
        "tooling": {"blockbench_version": "5.1.6", "transport": "CDP_LOOPBACK_ONLY", "endpoint": "http://127.0.0.1:9266", "extensions_disabled": True},
        "texture_policy": "EXACT_32X32_PACKET_BYTES_PRESERVED_NO_RESAMPLE_OR_UPSCALE",
        "totals": totals,
        "assets": assets,
        "proof_boundaries": {
            "proves": ["BLOCKBENCH_5_1_6_NATIVE_EDITABLE_ROUNDTRIP", "EXACT_BRIEF_CLIP_SET", "TRUE_NATIVE_EFFECT_LOCATORS", "EDITABLE_SHAPE_UV_PRESERVATION", "EXACT_PACKET_TEXTURE_BYTES", "TWO_NATIVE_EXPORT_CYCLES", "TIMELINE_PROOF_PER_DECLARED_CLIP"],
            "does_not_prove": ["BP_RP", "ICONS", "GAMEPLAY", "AUTHORITY", "BDS", "BEDROCK_CLIENT", "MULTIPLAYER", "PHYSICAL_PS4", "MARKETPLACE", "RELEASE"],
        },
    }
