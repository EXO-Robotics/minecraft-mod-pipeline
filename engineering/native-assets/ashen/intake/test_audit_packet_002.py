...
import audit_packet_002 as audit


HERE = Path(__file__).resolve().parent
REPORT_PATH = HERE / "ASHEN_PACKET_002_NATIVE_READINESS.json"
MARKDOWN_PATH = HERE / "ASHEN_PACKET_002_NATIVE_READINESS.md"


class Packet002NativeReadinessTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_exact_authority_and_scope(self):
        self.assertEqual(
            self.report["audit_scope"]["integration_commit"],
            "9acf1b0f62ade90b59ba65e0a9e0618852ff3159",
        )
        self.assertTrue(self.report["audit_scope"]["read_only_packet_audit"])
        self.assertFalse(self.report["audit_scope"]["blockbench_launched"])
        self.assertFalse(self.report["audit_scope"]["packet_assets_edited"])
        self.assertFalse(self.report["audit_scope"]["bp_rp_touched"])
    ...
    def test_native_blockers_are_measured_not_promoted(self):
        ...
        self.assertEqual(summary["texture_contract_compatible"], 2)
        self.assertEqual(summary["texture_contract_mismatch"], 48)
    ...
    def test_blockbench_disposition_is_class_specific(self):
        ...
        self.assertTrue(all(asset["shipping_readiness"] == "STATIC_TEXTURE_NORMALIZATION_REQUIRED" for asset in not_applicable))
    ...
    def test_representative_gate_and_repair_order_are_bounded(self):
        ...
        self.assertEqual(
            representatives,
            ["ash_drake", "ember_crow", "ash_ram", "fire_bloom", "smoke_reed", "ember_forge", "ancient_kiln"],
        )
