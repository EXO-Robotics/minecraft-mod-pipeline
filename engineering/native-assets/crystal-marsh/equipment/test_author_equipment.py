...
EXPECTED = ["crystal_pike", "prism_bow", "crystal_circlet", "explorer_cloak", "crystal_shovel", "marsh_sickle", "crystal_talisman", "marsh_idol", "marsh_wight_mask", "moon_pearl_pedestal", "crystal_obelisk_fragment"]


class CrystalEquipmentNativeTest(unittest.TestCase):
    def test_scope_is_exact_and_deferred_adjacent_items_absent(self):
        self.assertEqual(list(author.SPECS), EXPECTED)
        self.assertNotIn("surveyor_staff", author.SPECS)
        self.assertNotIn("trail_compass", author.SPECS)
        self.assertEqual(sum(len(row["clips"]) for row in author.SPECS.values()), 16)
    ...
    def test_effect_locator_uses_canonical_export(self):
        for asset in author.SPECS:
            ...
            self.assertEqual(plan["effect"]["parent"], exported["effect"]["source_parent"])
            self.assertEqual(plan["effect"]["position"], exported["effect"]["position"])
    ...
    def test_receipts_if_present(self):
        ...
        for asset, spec in author.SPECS.items():
            ...
            self.assertEqual(receipt["schema"], "aionforge.wave1.crystal_marsh.equipment_native.v1")
    ...
    def test_aggregate_if_present(self):
        ...
        self.assertEqual(report["totals"], {"assets": 11, "brief_declared_clips": 16, "true_native_locators": 11, "screenshots": 104, "warnings": 0, "errors": 0})
        self.assertEqual(report["excluded_and_unchanged"]["W1-CREATIVE-005"], "DEFERRED_NO_SIDEGRADES_AUTHORED")
