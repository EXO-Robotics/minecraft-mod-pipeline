...
class CrystalMarshPlantContractTest(unittest.TestCase):
    def test_scope_is_exact_and_representatives_are_excluded(self):
        ...
        self.assertNotIn("bubble_pod", author.ASSETS)
        self.assertNotIn("flood_reed", author.ASSETS)
    ...
    def test_existing_receipts(self):
        ...
        for asset, spec in author.SPECS.items():
            ...
            self.assertFalse(receipt["scope_enforcement"]["representative_bubble_pod_edited"])
            self.assertFalse(receipt["scope_enforcement"]["representative_flood_reed_edited"])
            total_clips += len(spec["clips"])
        self.assertEqual(total_clips, 5)
    ...
    def test_aggregate_is_deterministic_and_bounded(self):
        ...
        self.assertEqual(actual["excluded_representatives"], ["bubble_pod", "flood_reed"])
        self.assertEqual(actual["totals"], {"assets": 8, "brief_declared_clips": 5, "true_native_locators": 8, "screenshots": 69, "warnings": 0, "errors": 0})
