#!/usr/bin/env python3
"""Build the deterministic aggregate Crystal Marsh plant native receipt."""
...
from author_plants import ASSETS, INTEGRATION_COMMIT, INTEGRATION_TREE, RECEIPT_NAME


HERE = Path(__file__).resolve().parent
OUTPUT = HERE / "CRYSTAL_MARSH_PLANT_NATIVE_REPORT.json"
...
def build() -> dict:
    ...
    for asset in ASSETS:
        ...
        assets.append({
            ...
            "authored_clip_names": receipt["authored_clip_names"],
    ...
    return {
        "schema": "aionforge.wave1.crystal_marsh.plant_native.aggregate.v1",
        "status": "PASS_NATIVE_REPAIR_GATE",
        "integration_authority": {"commit": INTEGRATION_COMMIT, "tree": INTEGRATION_TREE},
        "scope": list(ASSETS),
        "excluded_representatives": ["bubble_pod", "flood_reed"],
        "tooling": {"blockbench_version": "5.1.6", "transport": "CDP_LOOPBACK_ONLY", "extensions_disabled": True},
        "totals": {
            "assets": len(assets),
        ...
        "proof_boundaries": {
            "proves": [
                ...
                "EXACT_BRIEF_DECLARED_CLIP_AUTHORING_WITHOUT_GENERIC_ALIASES",
                ...
                "NATIVE_FIXED_VIEWS_AND_TIMELINE_PROOF_PER_DECLARED_CLIP",
                "ZERO_WARNING_ZERO_ERROR_NATIVE_SESSIONS",
            ],
            "does_not_prove": [
                "BP_RP_INTEGRATION", "GAMEPLAY", "CREATOR_TOOLS", "BDS", "BEDROCK_CLIENT",
                "MULTIPLAYER", "PHYSICAL_PS4", "MARKETPLACE", "RELEASE",
            ],
