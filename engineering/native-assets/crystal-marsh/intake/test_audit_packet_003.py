...
import audit_packet_003 as audit


HERE = Path(__file__).resolve().parent
REPORT_PATH = HERE / "CRYSTAL_MARSH_PACKET_003_NATIVE_READINESS.json"
MARKDOWN_PATH = HERE / "CRYSTAL_MARSH_PACKET_003_NATIVE_READINESS.md"


class Packet003NativeReadinessTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_exact_authority_and_scope(self):
        ...
        self.assertEqual(
            scope["integration_commit"],
            "bcd65076900a3688dd797d54719263d88afd501c",
        )
        self.assertEqual(
            scope["integration_tree"],
            "4d876b233e6b510687d238f1d7f6611c7c0c4ab9",
        )
    ...
    def test_complete_exact_inventory_and_hashes(self):
        ...
        self.assertEqual(
            summary["tier_counts"],
            {"BLOCK": 10, "CREATURE": 10, "LANDMARK": 10, "PLANT": 10, "RESOURCE": 10},
        )
    ...
    def test_native_blockers_are_measured_not_promoted(self):
        ...
        self.assertEqual(summary["texture_contract_compatible"], 3)
        self.assertEqual(summary["texture_contract_mismatch"], 47)
        ...
        self.assertTrue(
            all(asset["native_export_equivalence"] == "UNPROVEN" for asset in self.report["assets"])
        )

    def test_blockbench_disposition_is_class_specific(self):
        native = [
            asset for asset in self.report["assets"]
            if asset["blockbench_disposition"] == "NATIVE_REPAIR_REQUIRED"
        ]
        not_applicable = [
            asset for asset in self.report["assets"]
            if asset["blockbench_disposition"].startswith("NOT_APPLICABLE")
        ]
        ...
        self.assertTrue(
            all(asset["shipping_readiness"] == "STATIC_TEXTURE_NORMALIZATION_REQUIRED" for asset in not_applicable)
        )

    def test_exact_representative_gaps_are_bound(self):
        by_name = {asset["name"]: asset for asset in self.report["assets"]}
        wight = by_name["marsh_wight"]
        comparison = wight["declared_vs_actual"]
        ...
        self.assertEqual(
            comparison["missing_declared_role_clips"],
            ["death_collapse", "drift_walk", "hurt", "idle_sway", "reach_attack"],
        )
        self.assertEqual(comparison["generic_or_extra_exported_clips"], ["action", "idle"])
        self.assertEqual(comparison["texture_contract"], "64×64")
        self.assertEqual(comparison["decoded_texture_resolution"], [32, 32])
        self.assertTrue(wight["path_normalization"]["requires_relative_shipping_path_rebind"])
        self.assertEqual(
            [term["resolved_packet_warehouse_ids"] for term in wight["related_asset_bindings"]["terms"]],
            [["sunken_shrine"], ["moon_pearl"]],
        )
    ...
    def test_representative_gate_and_repair_order_are_bounded(self):
        ...
        self.assertIn("Blockbench-N/A", stages[0]["scope"])
    ...
    def test_committed_reports_are_deterministic(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            two.write_text(
                json.dumps(audit.build_report(audit.DEFAULT_PACKET_ROOT), indent=2, sort_keys=True) + "\n",
                encoding="utf-8",
            )
