...
def build():
    files={}; blocks=json.loads((REPO/"resource_pack/blocks.json").read_text()); terrain=json.loads((REPO/"resource_pack/textures/terrain_texture.json").read_text()); lang=(REPO/"resource_pack/texts/en_US.lang").read_text().splitlines(); prefixes=tuple(f"tile.aionbound:{a}.name=" for a in ASSETS); rows=[]
    for spec in SPECS:
        ...
        files[gp]=geo.read_bytes(); files[tp]=tex.read_bytes(); files[bp]=encoded(block(spec)); files[fp]=encoded(feature(spec)); files[rp]=encoded(rule(spec)); blocks[f"aionbound:{spec.asset}"]={"sound":"grass","textures":spec.asset}; terrain["texture_data"][spec.asset]={"textures":f"textures/aionbound/skyreach/plants/{spec.asset}"}
    ...
    replacements={f"tile.aionbound:{a}.name":f"tile.aionbound:{a}.name={DISPLAY[a]}" for a in ASSETS}; emitted=set(); stable_lang=[]
    for line in lang:
        key=line.split("=",1)[0] if "=" in line else None
        if key in replacements:
            if key not in emitted: stable_lang.append(replacements[key]); emitted.add(key)
        else: stable_lang.append(line)
    stable_lang.extend(replacements[key] for key in sorted(replacements.keys()-emitted))
