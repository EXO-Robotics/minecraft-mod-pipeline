...
Preview 1.26.50.20 synthetic `useItemInSlotOnBlock` testing verifies the GameTest pack's stable before-event observation, while the separate production pack still receives no contract-valid player actor. The production handler denies that malformed player-owned action without throwing. The same run verifies a real vanilla husk damaging the SimulatedPlayer and a generated boss entering phase one through production entity lifecycle registration and the generated scheduler. These synthetic checks are narrow integration evidence; a physical stable client is still required.
