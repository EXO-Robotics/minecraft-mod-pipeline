...
Preview 1.26.50.20 synthetic testing verifies stable item/block events while the separate production pack still receives no contract-valid actor for player-owned GameTest actions; those actions fail closed. Independent fixtures use explicit projectile vectors and separately prove entity and block collision, while direct effect checks prove stable API acceptance plus immediate/delayed speed observation on both SimulatedPlayer and a normal entity. Generated lifecycle logs prove entity growth, bounded machine output, and all three boss phases. A second clean boot observes the diagnostic checkpoint. These checks remain narrow integration evidence; generated real-player launcher/form behavior, multiplayer, and physical stable clients remain required.
