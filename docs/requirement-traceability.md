...
| DOC-03 | AI tool interface | `IMPLEMENTED` operation catalog; external execution `PARTIAL` | All 79 required names dispatch, including diagnostic-only reconstruction-wave diagnosis, clean-room reconstruction-wave preparation, and autonomous Blockbench asset authoring; persistent artifact-backed workflow and safe edits run, while unmanaged runtime operations fail closed |
...
| DOC-12 | Persistence/migrations | `DOCUMENTED`; implementation `PARTIAL` | Benchmark B validates revisioned records, normalizes sparse records, recovers a prepared-journal interruption while migrating one nonempty v0 lock through a state-preserving BDS pack upgrade, and reads it on a third boot; other storage boundaries, reconnect, player-created state and machine restoration remain unproven |
...
| API-03 | Fail-closed event adapters | `PARTIAL` | One authoritative typed registry covers every recognized trigger, condition, action, nested action, explicit recipient, owner-specific inventory route, and required actor/target/block/item/projectile/dimension/event-source context; unknown vocabulary fails compilation. Generated adapters normalize hurt/death plus independent entity/block projectile impacts, use stable block before-events, reacquire scheduled blocks, and defer mutable work. Preview evidence remains non-client and non-console |
...
| VAL-01 | Current repository unit suite | `IMPLEMENTED` | 150 tests pass; one compiled-Java test is skipped because this host has no usable JDK; strict mypy passes 61 source/tool files |
| VAL-02 | Real-action event integration | `PARTIAL` | Exact Preview BDS 1.26.50.20 diagnostics cleanly cover DoorLock before/after block break plus 26 independent Clockwork checks: item use/use-on-block, block interaction, projectile creation/entity impact/block impact/cleanup, direct effect invocation and immediate/delayed observation on SimulatedPlayer and a normal entity, progression storage, bounded generated machine output, entity spawn/growth, melee hit/hurt/death, all three boss phases, and restart checkpoint recovery. The separate stable pack still receives no contract-valid actor for GameTest player-owned actions and rejects them. Preview evidence is not physical gameplay, Marketplace, or console evidence |
| VAL-03 | Persistence/multiplayer/migration | `PARTIAL` runtime | Revision-checked state, schema validation, pure adverse migration cases, prepared-journal BDS recovery, a nonempty upgrade, migrated-record restart, and narrow diagnostic persistence pass; player actions, other interruption boundaries, reconnect, and multiplayer remain unverified |
...
| BENCH-A | Original Marketplace showcase | `IMPLEMENTED` static benchmark; expanded BDS diagnostic `PASSED` | Reproducible builder applies evidence-linked runtime overrides across the executable fixture. Byte-identical clean builds pass Creator Tools 0.17.6 and exact stable BDS boot; the excluded two-cycle Preview world passes 26 narrow independent adapter/API/simulated checks including both projectile collision paths, effects, machine output, growth, all boss phases, and diagnostic restart recovery. Real-client launcher dispatch, form presentation, multiplayer, controller, and platform qualification remain |
| BENCH-B | Rights-cleared real mod | technical reconstruction `PARTIAL`; rights `REVIEW_REQUIRED` | Pinned DoorLock evidence drives a deterministic clean-room package with lock controller, configured shared credentials, source-shaped iron/gold key recipes, canonical door/double-chest locations, source-default event-confirmed break cleanup, bounded loaded-block redstone reconciliation, revisioned migration, stable controller confirmation, complete unresolved fidelity records, Creator Tools pass, three-cycle stable-BDS upgrade proof, forced-open trapdoor adapter proof across restart, and one clean Preview SimulatedPlayer break-adapter test; physical-player gameplay, multiplayer, console fidelity, and human rights review remain |
