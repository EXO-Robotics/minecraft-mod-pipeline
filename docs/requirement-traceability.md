...
| BENCH-B | Rights-cleared real mod | technical reconstruction `PARTIAL`; rights `REVIEW_REQUIRED` | Pinned DoorLock evidence drives a deterministic clean-room package with lock controller, configured shared credentials, canonical door/double-chest locations, revisioned migration, stable controller confirmation, complete unresolved fidelity records, Creator Tools pass, and three-cycle BDS upgrade proof; gameplay fidelity and human rights review remain |
