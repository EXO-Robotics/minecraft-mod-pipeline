...
| VAL-01 | Current repository unit suite | `IMPLEMENTED` | 145 tests pass; one compiled-Java test is skipped because this host has no usable JDK; strict mypy passes 57 source files |
| VAL-02 | Real-action event integration | `PARTIAL` | Preview BDS 1.26.50.20 SimulatedPlayer performed a real block-break action that reached before/after break adapters cleanly; item use, use-on-block, hit, hurt, interaction, projectile, spawn, death, scheduled action, and physical-client coverage remain unverified, and Preview evidence is not Marketplace or console evidence |
