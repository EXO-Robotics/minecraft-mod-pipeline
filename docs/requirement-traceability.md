...
| API-03 | Fail-closed event adapters | `PARTIAL` | Unmapped required triggers fail compilation; generated adapters copy durable event context, reject missing required actors/targets, normalize hurt/death/projectile shapes, and defer mutable work. Preview SimulatedPlayer evidence covers item-use observation, entity spawn, projectile impact, melee hit/hurt/death, and block break; use-on-block, block interaction, entity-damages-player, scheduled behavior, and physical-client execution remain untested |
...
| VAL-01 | Current repository unit suite | `IMPLEMENTED` | 149 tests pass; one compiled-Java test is skipped because this host has no usable JDK; strict mypy passes 61 source/tool files |
| VAL-02 | Real-action event integration | `PARTIAL` | Exact Preview BDS 1.26.50.20 diagnostics cleanly cover DoorLock before/after block break plus Clockwork Gardens item-use observation, generated entity-spawn phase write/read, generated projectile-hit action, and real melee hit/hurt/death. The GameTest-created player's item source is unavailable to the separate stable pack and is rejected rather than dispatched at a fallback location. Use-on-block, block interaction, entity-damages-player, scheduled behavior, and all physical-client coverage remain unverified; Preview evidence is not gameplay, Marketplace, or console evidence |
...
| BENCH-A | Original Marketplace showcase | `IMPLEMENTED` static benchmark; BDS diagnostic `PARTIAL` | Reproducible builder applies ten evidence-linked runtime overrides across the executable 21-family fixture. The clean stable archive passes Creator Tools and exact stable BDS boot; the excluded Preview world passes narrow item-use, entity-spawn, projectile-impact, and melee adapter checks. Persistence, multiplayer, controller, physical-player, and platform qualification remain |
