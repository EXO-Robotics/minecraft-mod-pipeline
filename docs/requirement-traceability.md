...
| API-03 | Fail-closed event adapters | `PARTIAL` | Unmapped required triggers fail compilation; generated adapters copy durable event context, reject missing required actors/targets, normalize hurt/death/projectile shapes, use the stable before-event for block use, reacquire scheduled blocks by dimension/location, and defer mutable work. Preview SimulatedPlayer evidence covers item use/use-on-block, block interaction, entity spawn, projectile impact, melee hit/hurt/death, hostile entity damage, scheduled boss behavior, and block break; physical-client execution remains untested |
...
| VAL-02 | Real-action event integration | `PARTIAL` | Exact Preview BDS 1.26.50.20 diagnostics cleanly cover DoorLock before/after block break plus Clockwork Gardens item use/use-on-block, machine block interaction, generated entity-spawn and scheduled boss phase writes, generated projectile-hit action, real melee hit/hurt/death, and a vanilla husk damaging the SimulatedPlayer. The GameTest-created player's actor is unavailable to the separate stable pack for player-owned actions and is rejected rather than dispatched at a fallback location. All physical-client coverage remains unverified; Preview evidence is not gameplay, Marketplace, or console evidence |
...
| BENCH-A | Original Marketplace showcase | `IMPLEMENTED` static benchmark; BDS diagnostic `PARTIAL` | Reproducible builder applies ten evidence-linked runtime overrides across the executable 21-family fixture. The clean stable archive passes Creator Tools and exact stable BDS boot; the excluded Preview world passes narrow item/use-on-block, block interaction, entity-spawn, scheduled boss, projectile-impact, bidirectional damage, and melee adapter checks. Persistence, multiplayer, controller, physical-player, and platform qualification remain |
