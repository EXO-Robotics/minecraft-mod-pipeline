...
Require a standardized assignment for role `independent_auditor`, skill
`audit-java-bedrock-cleanroom`, and lane `AUDIT`. Validate it with
`$translate-java-mods-to-bedrock`'s
role-contract validator. Audit is read-only; repairs require new production or
...
Audit product reachability separately from semantic model coverage. Trace each
promoted feature from normal acquisition/spawn through the public Bedrock event
or component, authoritative handler, state transition, result, and cleanup.
Exported functions, test-only summons, and direct internal oracle calls do not
prove player reachability.
...
Require `$qualify-bedrock-candidate` receipts for the exact final candidate.
Independently verify their hashes, authority, classifications, raw logs, and
limitations. Do not rerun or repair BDS while acting as semantic auditor.

Rebuild the exact frozen candidate twice. Run static and asset validation and
Creator Tools where available. Confirm the qualifier separately exercised
Stable BDS, Preview BDS, assigned multiplayer diagnostics, restart, persistence,
cleanup, and worst-credible stress. A marker pack proves Script API
availability, not candidate gameplay callbacks.
Require the candidate module to import and register every pinned stable event on
both server channels. Keep real client interaction delivery pending when only
BDS fixtures or direct module tests invoke the behavior.
